...
class CustomDomainRequestRewriteFilterTest {
  ...
  // verified custom domain. One parameterized test (js/css/fonts) instead of three near-identical
  // copies — same exclusion path, only the request path itself differs between cases.
  @ParameterizedTest
  @ValueSource(
      strings = {
        "/js/login-submit-guard.js",
        "/css/clavaris.css",
        "/fonts/geistSans-Regular.woff2"
      })
  void neverRewritesStaticAssetRequestsEvenOnAVerifiedCustomDomain(final String staticAssetPath)
      throws Exception {
    ClientDomainConfigRepository domainConfigs = mock(ClientDomainConfigRepository.class);
    OAuthClientRepository oauthClients = mock(OAuthClientRepository.class);
    CustomDomainRequestRewriteFilter filter =
        new CustomDomainRequestRewriteFilter(domainConfigs, oauthClients);
    MockHttpServletRequest request = new MockHttpServletRequest("GET", staticAssetPath);
    request.setServerName("login.example.com");
    MockHttpServletResponse response = new MockHttpServletResponse();
    MockFilterChain chain = new MockFilterChain();
