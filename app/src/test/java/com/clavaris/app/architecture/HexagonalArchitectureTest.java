...
/**
 * Enforces CLAUDE.md §7.2's dependency rule as a real, CI-checked constraint —
 * not a code-review convention. Runs against every business module's compiled
 * classes at once ("app" is the one module that depends on all of them).
 *
 * On zero domain classes (current state, CLAUDE.md §11), every rule below
 * holds vacuously — the point is that the guardrail exists from day one, so
 * the FIRST class ever added to domain/ is already checked, not the
 * hundredth.
 */
