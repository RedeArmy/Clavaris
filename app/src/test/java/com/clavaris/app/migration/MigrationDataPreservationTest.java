...
/**
 * Worked example, not a test of real schema (data-model.md §4 — no bounded
 * context owns real migrations yet). Proves the methodology every future
 * migration that ALTERS an existing table must follow: apply schema up to
 * N-1, seed representative data, apply migration N, assert the data is
 * still there and correctly transformed. A migration that only ever runs
 * against an empty database (every test elsewhere in this project so far)
 * never actually proves this — this class exists specifically to close
 * that gap.
 *
 * The migrations under test live in src/test/resources/db/migration-pattern-
 * example, deliberately separate from the real app/src/main/resources/db/
 * migration history — this is a template to copy per future migration that
 * touches existing data, not itself part of what ships.
 */
@Testcontainers
    ...
    void columnRenameMigrationPreservesExistingRows() throws Exception {
        ...
        try (Connection connection = DriverManager.getConnection(
            ...
            statement.executeUpdate(
                    "INSERT INTO migration_pattern_demo (id, legacy_status) VALUES ('" + seededId + "', 'ACTIVE')");
        }
