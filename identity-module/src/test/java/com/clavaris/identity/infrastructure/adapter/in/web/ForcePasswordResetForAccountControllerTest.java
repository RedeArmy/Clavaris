...
class ForcePasswordResetForAccountControllerTest {
  ...
  private ForcePasswordResetForAccountUseCase useCase;
  private MockMvc mockMvc;

  @BeforeEach
  void setUp() {
    useCase = mock(ForcePasswordResetForAccountUseCase.class);
    mockMvc =
        MockMvcBuilders.standaloneSetup(new ForcePasswordResetForAccountController(useCase))
            .build();
  }
  ...
  void returns204AndPassesThePlatformClientActorThrough() throws Exception {
    mockMvc
        .perform(
            post("/api/v1/admin/accounts/" + accountId + ":force-password-reset")
                .principal(ACTING_PLATFORM_CLIENT))
        .andExpect(status().isNoContent());

    ArgumentCaptor<ForcePasswordResetForAccountCommand> command =
        ArgumentCaptor.forClass(ForcePasswordResetForAccountCommand.class);
