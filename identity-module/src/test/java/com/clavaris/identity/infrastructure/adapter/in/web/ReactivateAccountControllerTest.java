...
class ReactivateAccountControllerTest {
  ...
  @BeforeEach
  void setUp() {
    useCase = mock(ReactivateAccountUseCase.class);
    mockMvc = MockMvcBuilders.standaloneSetup(new ReactivateAccountController(useCase)).build();
  }
  ...
  void returns204AndPassesThePlatformClientActorThrough() throws Exception {
    ...
    ArgumentCaptor<ReactivateAccountCommand> command =
        ArgumentCaptor.forClass(ReactivateAccountCommand.class);
