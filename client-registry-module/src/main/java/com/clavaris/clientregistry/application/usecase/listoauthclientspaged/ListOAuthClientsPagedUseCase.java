 ...
 * ListOAuthClientsUseCase} — that use case stays unbounded and remains the one {@code
 * PlatformOAuthClientController} uses for its own anti-enumeration ownership check
...
public interface ListOAuthClientsPagedUseCase {
  ...
  KeysetPage<OAuthClient> handle(ListOAuthClientsPagedQuery query);
}
