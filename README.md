...
- `organization-module` — `Organization` (tenant isolation boundary, one per consuming system, implemented) + two-layer rate-limit policy management. `Workspace` (team/company grouping within one Organization — the old "organization" concept, renamed by ADR-0010) is designed (ADR-0010) but has no code yet.
