...
- ~~Backup/disaster-recovery story for PostgreSQL (accounts, signing keys' metadata, organizations)~~ — **closed and live-verified 2026-09-08 (TD-FUT-006)**: `scripts/host/backup-postgres.sh`/`restore-postgres.sh` (a real `pg_dump`/`pg_restore` cycle plus the `clavaris-signing-keys-data` volume, restoring one without the other would leave `SigningKey` rows pointing at key material no longer on disk), `deployment-runbook.md` §6. Live-tested end to end against a real `postgres:16` container: seeded representative data, genuinely destroyed it, restored it, confirmed byte-identical recovery — not just written and trusted. **Named residual, not silently solved**: backups currently live on the same disk as the data they protect — no off-site/geo-redundant copy exists yet, tracked as TD-FUT-029.
- **Graceful degradation when a social login provider (Google, GitHub) is down** — sharpened 2026-09-08 (TD-PROC-002 continuation, SDE-III review): the real, concrete shape of this gap is a social-only `Account` (zero password credential, confirmed structurally possible since `JpaAccountRepository#save` only persists a `PasswordCredential` row when the aggregate actually carries one — see `roadmap-and-release-plan.md`'s own 2026-08-30 social-login closure note). Unlike ADR-0024's password-optional sign-up policy (BR-ID-19), which deliberately attaches a hidden, never-surfaced random password specifically to avoid this exact zero-auth-method trap, social-only sign-up (ADR-0020) has no equivalent fallback — an account created via Google-only sign-up has no way to authenticate at all if Google is down, full stop, not "degraded," genuinely locked out. Tracked as **TD-FUT-030**, not left as an undecided abstraction any longer.
