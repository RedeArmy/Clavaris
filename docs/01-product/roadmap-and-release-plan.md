...
| Rate-limit observability (per-rule/per-endpoint allow/block metrics and alerting) | `technical-debt-register.md` TD-FUT-011 — stack now chosen (**ADR-0015**, 2026-08-24: Micrometer + self-hosted Prometheus/Alertmanager/Grafana, mirroring JobSeeker's own choice), implementation deliberately deferred to its own pass rather than bolting metrics onto one feature ahead of the rest |
