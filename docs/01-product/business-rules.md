...
- **BR-ID-13** (2026-08-31) — Every tenant `Account` can self-service list and individually revoke its own live hosted-login sessions (`GET`/`POST /o/{organizationId}/account/sessions`). Listing/revocation is always scoped to the caller's own resolved `AccountId` from the authenticated session — never by a client-supplied identifier — so one Account can never see or revoke another's session, even by guessing a session id (`RevokeAccountSessionService`'s own ownership check). v1 scope: tenant `Account` only, not `PlatformAccount`.
- **BR-ID-14** (2026-08-31) — A tenant `Account`'s first successful login from a given device (fingerprinted by raw `User-Agent`, not IP — see `KnownDevice`) triggers a "new sign-in" notification email to that Account's own address of record; a subsequent login from an already-seen device sends nothing. A failed notification send never fails the login itself (deliberately different from every other transactional email in this system — see `RecordAccountLoginDeviceService`'s own Javadoc). v1 scope: informational only, no "this wasn't me" action link; tenant `Account` only, not `PlatformAccount`.
