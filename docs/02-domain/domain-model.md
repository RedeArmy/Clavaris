...
- **`Session.scopes`** — real (not placeholder) shape: fixed at open, since RFC 6749 §6 forbids a refresh grant from ever widening scope beyond what was originally authorized. Replaces this diagram's original placeholder `userAgent` field — this aggregate still deliberately doesn't carry one (BR-ID-03 never needed it); the "list your active sessions/devices" feature (BR-ID-13, shipped 2026-08-31, mirrored for `PlatformAccount` 2026-09-02) is built on a different, already-live mechanism instead — the Spring-Session-backed `HttpSession` store, not this aggregate — see `AccountSessionsController`'s own Javadoc for why the two must not be conflated.
