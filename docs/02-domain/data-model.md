...
- **`organizations`** — ADR-0010: one row per consuming system (tenant isolation boundary), not to be confused with `workspaces` below. Owns its own isolated `accounts` and `oauth_clients`. **`environment`/`linked_environment_organization_id`** (SDE-III feature build, 2026-09-04, Clerk Development/Production instances analysis): `environment` (`varchar(20)`, `DEFAULT 'PRODUCTION'`) distinguishes a sandboxed Organization from a real, unbounded one — every already-existing row defaults to `PRODUCTION` at the database level (migration `V20260904090000`), while `Organization.register()` (the Java-side default for every *new* Organization going forward) explicitly constructs `DEVELOPMENT`. `linked_environment_organization_id` is a nullable self-reference (`REFERENCES organizations (id)`) to the paired sibling Organization — populated on both sides once `:create-production-environment` has been used. Neither column changes the isolation model itself: a `DEVELOPMENT` Organization gets exactly the same structural isolation (its own issuer, JWKS, account pool) any other Organization already gets, only the *policy* differs (a real, explicit, low-default `rate_limit_policies` row; `RequestEmailVerificationService`/`RequestPasswordResetService` skip the real outbound send). **Updated 2026-09-24**: `OAuthClient.client_id` no longer encodes environment at all (previously `test_`/`live_`-prefixed) — see `business-rules.md` BR-ORG-08's own update for why.
...
- **`sessions.scopes`** — BR-ID-03, real (not placeholder) shape: `text` (JSON array, same convention as `oauth_clients.allowed_scopes`), fixed at open time — RFC 6749 §6 forbids a refresh grant from ever widening scope beyond what the original authorization actually granted, so every `refresh_tokens` row in this session's chain is validated against this same, unchanging set. Replaces this document's original placeholder `user_agent` column — still no use case populates one; the "list your active sessions/devices" feature this note used to say didn't exist yet (BR-ID-13) now does, for both tiers (2026-09-02, TD-FUT-026 extended it to `PlatformAccount`), but it's built on the Spring-Session-backed `HttpSession` store instead (no table — Redis-only, see `SessionDeviceAttributes`), not on this table.
- **`known_devices`** (BR-ID-14, new-device login email notification) — `account_id` FK (`REFERENCES accounts (id) ON DELETE CASCADE`, from day one — unlike the social-login tables, which needed a follow-up migration to add this), `user_agent varchar(512)` (the raw header, display/audit only — never the match key), `device_token_hash varchar(255)` (TD-SEC-033, 2026-08-31: the real match key — the hash of an opaque, high-entropy, `HttpOnly` cookie this system mints, `DeviceCookie`; the original `UNIQUE(account_id, user_agent)` constraint was dropped the same day a device-recognition design keyed on a client-controlled, spoofable header could never safely keep — see `KnownDevice`'s own Javadoc), `first_seen_at`/`last_seen_at timestamptz`. `UNIQUE(account_id, device_token_hash)`. Deliberately a separate table from `sessions`/`HttpSession` — a device fingerprint needs to outlive any one 30-minute-TTL `HttpSession`, which gets a brand-new id on every login even from the same physical browser.
- **`platform_known_devices`** (BR-ID-14, TD-FUT-026, 2026-09-02) — the platform-tier mirror of `known_devices`, same shape exactly (`device_token_hash`-keyed from day one, no two-migration history to repeat), `platform_account_id` FK (`REFERENCES platform_accounts (id) ON DELETE CASCADE`) instead of `account_id`, no `organization_id` at all (same rationale `platform_accounts` itself already establishes — this identity belongs to no Organization). `UNIQUE(platform_account_id, device_token_hash)`.
...
- **`client_domain_configs`** — 🟡 proposed, implemented 2026-09-05, see ADR-0009. One-to-one with `oauth_clients` (unique `oauth_client_id`), same "optional, never a nullable column on the core entity" convention as `password_credentials`. `hostname` is globally unique (two clients can't claim the same subdomain — `ux_client_domain_configs_hostname`). `mode` is `CNAME | PROXY`; `verification_status` is `PENDING | VERIFIED | FAILED`; BR-CLIENT-04 requires `VERIFIED` before the embedded login experience is usable in production. `dns_txt_challenge_token` is the value an operator publishes as a TXT record to prove ownership (`VerifyClientDomainOwnershipService`, a real DNS lookup, admin-triggered). `embedding_origin` is the consumer's own frontend origin allowed to embed the hosted login in an iframe (`display=modal`) — independent of the domain-ownership fields above; re-requesting a domain (a changed `hostname`/`mode`) always resets `verification_status` to `PENDING` and mints a fresh `dns_txt_challenge_token`, but changing `embedding_origin` alone never does.
- **`client_brandings`** — 🟡 proposed, implemented 2026-09-05, see ADR-0009. One-to-one with `oauth_clients`; read by the Thymeleaf hosted-UI templates to theme the login/consent screens (`logo_url`, `primary_color`, `application_display_name`).
...
| `client_domain_configs` | unique `(oauth_client_id)`, unique `(hostname)` where `hostname IS NOT NULL` | one config per client; no two clients can claim the same subdomain |
