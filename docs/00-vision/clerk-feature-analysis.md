...
## 12. Correction, 2026-09-09 — §2 and §10.2 both understated Clavaris's own state, unnoticed until now

Found during an independent whole-codebase SDE-III review requested explicitly this session — not a self-audit of this file, a fresh reviewer checking this document's own claims against the real, compiled code before citing them, per this file's own §1 standard ("sourced," not asserted). The finding: **§2's original table and §10.2's own "refreshed" table (both still live above, neither edited) both say social login, webhooks, and user impersonation are unbuilt** — and all three have been fully shipped and live-verified for days to weeks, confirmed here by grepping the actual compiled classes, not re-reading either table:

- `app/src/main/java/.../SocialLoginConfig.java` + 3 supporting classes — real, wired, `SocialLoginIntegrationTest` green (already correctly recorded in `roadmap-and-release-plan.md` §2 as **shipped 2026-08-30**, ADR-0020).
- `webhook-module/src/main/java/.../WebhookDispatchScheduler.java` + a full `dispatchoutboxevents`/`deliverpendingwebhooks` use-case pair, real HMAC signing, retry, replay — GA per `technical-debt-register.md` §6 and `roadmap-and-release-plan.md` §18 (2026-09-08).
- `app/src/main/java/.../ImpersonateAccountController.java` + `ImpersonationTokenIssuer` + 4 more supporting classes, plus `identity-module`'s own `impersonateaccount` use case — shipped, per the same §18 entry (BR-ADMIN-01/02).

**Why this slipped past every prior pass of this specific file**: §11 (2026-09-08, one day before this correction) added new content without re-reading §2/§10.2 against current code — the same "add a new section, never re-verify the old ones" discipline this file itself follows correctly for *dated* history, applied here to content that was never actually current in the first place, not a snapshot of a past truth. `roadmap-and-release-plan.md` had the right answer the whole time (§2's own row-by-row updates, §18); it just never flowed back into this file's own tables. **Not fixing §2/§10.2 in place** — same reasoning as every other correction in this document: they're dated snapshots of what a 2026-08-12/2026-08-27 read of the codebase showed, and rewriting them would hide that this file's own review cadence missed a real drift for at least 10 days (2026-08-30 to today) despite an intervening edit (§11) that touched this same file. The lesson, not just the fix: **a section addition to this file is not itself a freshness check on the file's other tables** — a future edit here should explicitly re-diff §2/§10.2 against `roadmap-and-release-plan.md` §2's current row states before assuming either is still accurate, not just before publishing wholly new content.
