...
| No self-service way to retire the active `platform_signing_keys` row | §3b's forced-invalidation option (since TD-SEC-002 closed) is a raw-SQL `UPDATE` plus a restart, not a safe, auditable, one-click action | TD-SEC-010 (adjacent — same "no rotation endpoint" class of gap as the per-Organization case) |
| `PlatformClientRepository`/`OAuth2AuthorizationService` aren't checked per-request | Already-issued tokens survive credential rotation — bounded only by the 5-minute default TTL or a forced key-retirement-plus-restart, not by revoking the credential itself | TD-SEC-003 |

TD-SEC-002 (signing-key persistence) closed 2026-08-21, which is what turned §3b from "any restart mass-invalidates everyone's tokens" into the deliberate, opt-in procedure above — an improvement, but it also means §3b is no longer a side effect you get "for free" from §3a's own restart step; it now takes a distinct SQL action first. Revise this runbook the same day TD-SEC-018 (a real self-service rotation/revocation path for `PlatformClient`) ships — §3a's raw-SQL step is still the sharpest gap here.
