...
- Key-compromise incident response: **not yet written** — flagged as a known gap in `threat-model-stride.md` §5, required before the external security review gate. Now more urgent, not less: N organizations means N independent keys, so this runbook needs to specify per-tenant containment (rotate one Organization's key without affecting any other), not just a single-key procedure.
