...
- Tokens (refresh, verification, authorization codes): only their hash is persisted; the bearer value exists only in transit and on the holder's side (`data-model.md` §2). **Known, documented exception (TD-SEC-019):** `oauth2_authorization` — the table backing Spring Authorization Server's own `JdbcOAuth2AuthorizationService` (TD-SEC-003) — stores every access/refresh/ID token's literal bearer value, not a hash, because the framework's own `findByToken` lookup compares against it directly with no supported way to hash on write and still support that lookup. A Postgres compromise exposes these tokens directly; every other token this system controls the schema for still follows the hash-only rule.
