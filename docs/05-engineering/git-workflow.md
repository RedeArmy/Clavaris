...
Enforced by `.github/workflows/ci.yml`, triggered on push/PR against `master`, six substantive jobs plus one aggregator:
...
- `migration-immutability` — runs `scripts/check-migration-immutability.sh` (`definition-of-done.md` §1b); fails a PR that modifies, renames, or deletes a Flyway migration already merged to `master`, the exact class of change that broke pre-production on 2026-09-16 (a comment-only edit to an already-applied migration failed Flyway's checksum validation on the next deploy — nothing in this repo's own test setup, all fresh/empty Testcontainers databases, could have caught it before this check existed). `fetch-depth: 0`, same requirement as `sonarcloud` below — needs real history to compare against `origin/master`.
- `sonarcloud` — full analysis + Quality Gate (§4a below).
- `ci-passed` — depends on all six jobs above (`needs:`), `if: always()`, fails if any of them didn't succeed. Exists purely so GitHub branch protection has **one** status check to require instead of six — requiring each job by name individually breaks silently the moment any one of them is renamed (branch protection keeps waiting for the old name forever, with no obvious reason why a PR won't merge). Runs in parallel with nothing to wait on itself, so it costs no extra wall-clock time.
