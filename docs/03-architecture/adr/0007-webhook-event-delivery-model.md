...
Introduce a new bounded context, **`webhook-module`**, alongside the four modules in `CLAUDE.md` §4. It owns webhook endpoint registration, event subscription, signing, delivery, retry, and delivery history — it does **not** own the identity or organization domain logic that produces the events it delivers.
...
- **Positive:** Clavaris never gets network access to, or credentials for, a consumer's own database — the security boundary this whole project exists to keep clean (`CLAUDE.md` §1) stays intact.
