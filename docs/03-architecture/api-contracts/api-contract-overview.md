...
| `POST /api/v1/admin/accounts/{id}:impersonate` | BR-ADMIN-01/02: mints a short-lived, revocable Bearer access token (no refresh/ID token) for the target Account, scoped to one of its own Organization's `OAuthClient`s, carrying an RFC 8693 `act` claim — its own dedicated scope (`platform:accounts:impersonate`) and rate limit, shipped 2026-09-03 |
| `POST /api/v1/admin/organizations` | Create a new `Organization` (tenant isolation boundary, ADR-0010) — operator-only in v1, mirroring manual `OAuthClient` registration; provisions the initial `SigningKey`. Every new Organization defaults to `DEVELOPMENT` environment (Clerk Development/Production instances analysis, 2026-09-04) — a real, explicit, low-default `RateLimitPolicy` is provisioned for it; a `PRODUCTION` Organization (created via `:create-production-environment` below) still gets none, unchanged — a missing row still means "use the system default" |
| `POST /api/v1/admin/organizations/{id}:create-production-environment` | Promote a `DEVELOPMENT` Organization by creating its paired `PRODUCTION` sibling — same `platform:organizations:write` scope, structurally the same class of action as creating an Organization in the first place. Shipped 2026-09-04 |
...
| `POST /api/v1/admin/organizations/{id}/signing-keys/{kid}:purge` | TD-SEC-029: emergency, zero-overlap eviction of one *confirmed-compromised* key from JWKS, effective on the next read — its own `platform:signing-keys:purge` scope, deliberately separate from routine rotation above; generates and activates a replacement key first if the purged key was the active one, so the Organization is never left without one mid-incident |
